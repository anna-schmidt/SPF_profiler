...
master_times <- master_times %>%
  select(-c(datetime, day_datetime, month_datetime, year_datetime, time_datetime, time_profiledatetime, test,doy_datetime, doy_profiledatetime, depth,pH..mV.,chlorophyll..RFU.,phycocyanin..RFU.,oxygen.satuation,power,number.of.measurements,version,oxidation.reduction.potential,phycoeritrin,phycoeritrin..RFU.))

sum(is.na(master_times))
